...
function updateRunningText() {
    ...
    topGivers.forEach((giver, index) => {
        ...
        rankSpan.style.marginBottom = '2px';
...
if (topGivers.length > 0) {
    ...
    if (!top1Container) {
        ...
        top1Container.className = 'top-1-container'; // Menambahkan kelas float-up-down
    ...
    // Buat elemen <canvas> untuk efek batang visual
    const canvas = document.createElement('canvas');
    canvas.width = 400; // Perbesar ukuran canvas untuk lebih banyak batang
    canvas.height = 350;
    ...
    top1Container.appendChild(canvas);
    ...
    top1Img.style.width = '100px'; // Ukuran gambar opsional
    top1Img.style.height = '100px'; // Ukuran gambar opsional
    top1Img.style.borderRadius = '50%'; // Styling agar berbentuk lingkaran
    top1Img.style.marginTop = '15px';
    ...
    // Memanggil fungsi makeDraggable untuk top1Container
        ...
        // Setup Visualisasi Audio untuk gambar Top 1
        setupAudioVisualization(top1Img, crownImg);
}
...
//fungsi animasi musik
async function setupAudioVisualization(top1Img, crownImg) {
    try {
        // Meminta akses audio dari mikrofon pengguna
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
        const analyser = audioCtx.createAnalyser();
        analyser.fftSize = 128; // Ukuran FFT untuk analisis frekuensi
        ...
        // Membuat elemen canvas jika belum ada
        let canvas = document.querySelector('canvas');
        if (!canvas) {
            canvas = document.createElement('canvas');
            canvas.width = 450; // Lebar canvas
            canvas.height = 550; // Tinggi canvas
            document.body.appendChild(canvas);
        }
        const ctx = canvas.getContext('2d'); // Mendapatkan konteks 2d dari canvas

        // Ukuran gambar top1Img (180px), kita gunakan ini untuk radius lingkaran batang
        const imgSize = 100;
        const radius = imgSize / 2; // Setengah dari ukuran gambar, agar batang visualisasi pas di sekitar gambar

        function drawVisualizer() {
            requestAnimationFrame(drawVisualizer); // Memanggil diri sendiri untuk animasi berkelanjutan
            analyser.getByteFrequencyData(dataArray); // Mendapatkan data frekuensi audio

            // Bersihkan canvas setiap frame
            ctx.clearRect(0, 0, canvas.width, canvas.height);

            // Ambil frekuensi bass untuk efek denyut
            const bass = dataArray.slice(0, bufferLength / 4).reduce((a, b) => a + b, 0) / (bufferLength / 4);
            const scale = 1 + (bass / 400); // Skala untuk efek denyut, semakin tinggi bass semakin besar skala
            top1Img.style.transform = `translate(-50%, -50%) scale(${scale})`;
            crownImg.style.transform = `translate(-50%, -50%) scale(${scale})`;

            const centerX = canvas.width / 2;

            const offsetY = 45; // Menurunkan posisi visualisasi spektrum sebesar 50px
            const centerY = (canvas.height / 2) + offsetY;

            const halfLength = bufferLength / 2;

            // Gambar batang visualisasi spektrum
            for (let i = 0; i < halfLength; i++) {
                const angle = (i / halfLength) * Math.PI; // Sudut untuk sisi kiri
                const angleRight = (i / halfLength) * Math.PI + Math.PI; // Sudut untuk sisi kanan

                const length = (dataArray[i] / 255) * 70 + 20; // Panjang batang sesuai dengan data frekuensi

                // Koordinat titik awal dan akhir batang kiri
                const x1Left = centerX + Math.cos(angle) * radius;
                const y1Left = centerY + Math.sin(angle) * radius;
                const x2Left = centerX + Math.cos(angle) * (radius + length);
                const y2Left = centerY + Math.sin(angle) * (radius + length);

                // Koordinat titik awal dan akhir batang kanan
                const x1Right = centerX + Math.cos(angleRight) * radius;
                const y1Right = centerY + Math.sin(angleRight) * radius;
                const x2Right = centerX + Math.cos(angleRight) * (radius + length);
                const y2Right = centerY + Math.sin(angleRight) * (radius + length);

                // Menggambar batang visualisasi
                ctx.strokeStyle = `hsl(${i * 10}, 100%, 50%)`; // Warna batang berdasarkan indeks
                ctx.lineWidth = 5; // Ketebalan garis batang
                ctx.globalCompositeOperation = "lighter"; // Efek pencampuran cahaya untuk glowing
...
const imageElements = [
    '/images/donat.gif',      // GIF untuk Gift kecil mawar 0

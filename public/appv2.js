...
function toggleBlur(photoElement, uniqueId, img) {
    // Cek apakah uniqueId ada dalam list_penjahat dan hanya berikan efek blur pada foto tersebut
    if (list_penjahat.includes(uniqueId)) {
        photoElement.classList.toggle('blurred'); // Menambahkan atau menghapus kelas 'blurred'
    }
}
...
if (topGivers.length > 0) {
    ...
    top1Img.style.width = '180px'; // Ukuran gambar opsional
    top1Img.style.height = '180px'; // Ukuran gambar opsional
    top1Img.style.borderRadius = '50%'; // Styling agar berbentuk lingkaran
    top1Img.style.marginTop = '30px';
...
function addBubble(size, img, coint = 1, uniqueId = '') {
    ...
    // Menambahkan event listener untuk menangani klik pada foto
    photo.addEventListener('click', function () {
        toggleBlur(this, uniqueId, img);
    });
...
function loadAudio(input, index) {
        ...
        currentVolume = newVolume;  // Perbarui volume global
        audioElements.forEach(audio => {
            audio.volume = currentVolume;  // Atur volume untuk setiap audio
        });
    }

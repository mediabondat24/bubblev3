...
// let backendUrl = "https://bubblefoto.online/";
let connection;
...
connection.on('like', (msg) => {
    ...
    if (typeof msg.likeCount === 'number') {
        ...
        if (userLikes[msg.userId]) {
            userLikes[msg.userId].likeCount += msg.likeCount;  // Tambah jumlah like
        } else {
            userLikes[msg.userId] = {
                likeCount: msg.likeCount,
                profilePictureUrl: msg.profilePictureUrl || '/images/bubble.png', // Gunakan gambar default
                nickname: msg.nickname || 'RK Agency' // Gunakan nickname default jika tidak ada
            };
        }

        // Kirimkan data yang diperlukan ke dalam fungsi updateLeaderboard dan addLikeToAnimation
        updateLeaderboard();
        addLikeToAnimation(userLikes[msg.userId].profilePictureUrl, userLikes[msg.userId].likeCount, msg.nickname);
...
// Fungsi ini akan dipanggil saat pertama kali halaman dimuat
// Pastikan untuk memanggil fungsi makeDraggablelike setelah leaderboard update
function updateLeaderboard() {
    let leaderboardContainer = document.getElementById('leaderboard');
    
    // Clear current leaderboard items
    leaderboardContainer.innerHTML = '';

    // Loop through the userLikes object, sorted by the number of likes
    let sortedUsers = Object.keys(userLikes).sort((a, b) => userLikes[b].likeCount - userLikes[a].likeCount);
    
    // Show only the top 10 users
    sortedUsers.slice(0, 10).forEach((userId, index) => {
        let user = userLikes[userId];
        let leaderboardItem = document.createElement('div');
        leaderboardItem.className = 'leaderboard-item';

        leaderboardItem.innerHTML = `
            <div class="rank-number">${index + 1}</div>
            <img src="${user.profilePictureUrl}" class="profile-pic" alt="Profile Picture">
            <div class="text-container">
                <span class="nickname">${user.nickname}</span>
                <div class="like-container">
                    <i class="fa fa-thumbs-up"></i><span class="like-count">${user.likeCount}</span>
                </div>
            </div>
        `;
    ...
    // Terapkan fungsi draggable setelah leaderboard diupdate
    const topLikeElement = document.getElementById('toplike');
    makeDraggablelike(topLikeElement);
}
...
function addLikeToAnimation(profilePictureUrl, likeCount, nickname) {
    ...
    // Pastikan likeCount adalah angka yang valid, jika bukan angka set default 0
    ...
    newItem.innerHTML = `
        ...
        <div class="like-info">
            ...
            <span class="like-icon">
                ...
                <b>${validLikeCount}</b> <!-- Menampilkan total like yang valid -->
            </span>
        </div>
    `;
...
// Fungsi untuk membuat elemen draggable
    function makeDraggablelike(element) {
            let isDragging = false;
            let startX = 0, startY = 0;  // Untuk menyimpan posisi awal X dan Y
            let offsetX = 0, offsetY = 0; // Untuk menyimpan offset pergerakan
        ...
        // Panggil fungsi untuk elemen #toplike setelah halaman dimuat
...
function mvp() {
  ...
  maskot.src = '/images/sayap2.gif'; // Nama file gambar maskot
  maskot.alt = 'Maskot';
  maskot.style.width = '120px'; // Tentukan lebar gambar maskot
  maskot.style.height = '120px'; // Tentukan tinggi gambar maskot
  maskot.id = 'maskot';
  ...
  mvp.alt = 'mvp';
  mvp.style.width = '50px';  // Ukuran mvp lebih kecil dari maskot
  mvp.style.height = '50px'; // Ukuran mvp lebih kecil dari maskot
  mvp.style.top = '5px'; // Ukuran mvp lebih kecil dari maskot
  mvp.id = 'mvp';
...
window.onload = mvp; // Menetapkan Server 1 sebagai default saat halaman pertama kali dimuat
...
function renderTopGivers(top3) {
  ...
  top1Container.innerHTML = `
    <img src="/images/sayap4.gif" class="crown" alt="Crown"> <!-- Maskot tetap muncul pertama kali -->
 ...
 const top1ProfileImg = top1Container.querySelector('.profile');
  top1ProfileImg.style.width = '70px';  // Set custom width
  top1ProfileImg.style.height = '70px'; // Set custom height
  top1ProfileImg.style.top = '10px'; // Set custom height
  top1ProfileImg.style.objectFit = 'cover'; // Ensures the image fits within the box
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const crownSrc = i === 0 ? '/images/sayap1.gif' : '/images/sayap5.gif'; // Maskot untuk Top 2 dan Top 3
...
function updateTopGiversData(top3) {
  ...
  if (top3[0]) {
    top1.innerHTML = `
      <img src="/images/sayap4.gif" class="crown" alt="Crown">
    ...
    profileImg.style.width = '70px';  // Set custom width
    profileImg.style.height = '70px'; // Set custom height
    profileImg.style.objectFit = 'cover'; // Ensures the image fits within the box
  ...
  top3.slice(1, 3).forEach((giver, index) => {
    ...
    const crownSrc = index === 0 ? '/images/sayap1.gif' : '/images/sayap5.gif';  // Maskot untuk Top 2 dan Top 3
...
document.addEventListener("DOMContentLoaded", function() {
    fetch('/user/tiktok-data') 
        ...
        .then(data => {
            ...
            initializeLeaderboard();  // Pastikan ini dipanggil setelah data berhasil di-fetch
        })
        .catch(error => console.error('Error:', error));
});
